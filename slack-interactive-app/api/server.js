...
app.post('/slack/actions', (req, res) => {
  ...
  (async () => {
    try {
      ...
      if (payload.actions && payload.actions.length > 0) {
        ...
        if (action === 'button_add') {
          ...
          if (todaysDateString === ymd) {
            if (action === 'button_list') {
              ...
              action === 'button_office' ||
              action === 'button_remote'
            ) {
              // 本社勤務・在宅勤務ボタンクリック時
              try {
                ...
                // 選択した勤務体系を取得
                let workStyle =
                  action === 'button_office' ? 'office' : 'remote';
      ...
      } else if (payload.view?.callback_id === 'add_user_modal') {
        ...
        if (!users) {
          const { error } = await supabase
            .from('Users')
            .insert([{ code: userId, name: userName }]);
